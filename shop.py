...
def shop_inventory():
    ...
    sort_by = request.args.get('sort', 'latency')
    limit = min(int(request.args.get('limit', '25')), 100)
    offset = int(request.args.get('offset', '0'))
    items, total = pool.get_inventory(
        country=country or None, protocol=protocol or None,
        anonymity=anonymity or None, sort_by=sort_by,
        limit=limit, offset=offset)
    return jsonify({
        'proxies': [asdict(p) for p in items],
        'total': total,
        'limit': limit,
        'offset': offset,
    })
